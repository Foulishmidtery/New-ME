...
// 1. Proxy untuk Tax Holiday
apps.use(
  "/api/tax-holiday",
  createProxyMiddleware({
    // Menambahkan /api/tax-holiday di target agar path yang terpotong tersambung kembali
    target: "http://127.0.0.1:8000/api/tax-holiday",
    changeOrigin: true,
  }),
);

// 2. Proxy untuk Tax Allowance
apps.use(
  "/api/tax-allowance",
  createProxyMiddleware({
    // Menambahkan /api/tax-allowance di target
    target: "http://127.0.0.1:8000/api/tax-allowance",
    changeOrigin: true,
  }),
);

// 3. Proxy untuk Tax Super Deduction
apps.use(
  "/api/tax-super-deduction",
  createProxyMiddleware({
    target: "http://127.0.0.1:8000/api/tax-super-deduction",
    changeOrigin: true,
  }),
);

// ---------------- TAMBAHKAN INI ----------------
// 4. Proxy untuk Tax Bea Masuk
apps.use(
  "/api/tax-bea-masuk",
  createProxyMiddleware({
    target: "http://127.0.0.1:8000/api/tax-bea-masuk",
    changeOrigin: true,
  }),
);

const reactProjectPath =
  process.env.KNEKS_REACT_DIR || path.resolve(__dirname, "..", "kneks_react");
const reactAppPath = path.join(reactProjectPath, "quiz-kneks", "dist");
apps.use("/spes", express.static(reactAppPath));
apps.get("/spes/*", (req, res) => {
  res.sendFile(path.join(reactAppPath, "index.html"));
});

// 2. BODY PARSER HARUS DI BAWAH PROXY
...
// ---------------- INTEGRASI ZONA KHAS ----------------
// Opsional. Untuk development lokal, backend KHAS dapat dijalankan terpisah di port 4001.
if (process.env.ENABLE_KHAS_IN_CMS === "true") {
  const khasBackendPath = path.join(reactProjectPath, "backend-khas");
  const khasAuthRoutes = require(
    path.join(khasBackendPath, "src", "routes", "auth"),
  );
  const khasUsersRoutes = require(
    path.join(khasBackendPath, "src", "routes", "users"),
  );
  const khasPengajuanRoutes = require(
    path.join(khasBackendPath, "src", "routes", "pengajuan"),
  );
  const khasPenetapanRoutes = require(
    path.join(khasBackendPath, "src", "routes", "penetapan"),
  );

  apps.use("/api/khas/auth", khasAuthRoutes);
  apps.use("/api/khas/users", khasUsersRoutes);
  apps.use("/api/khas/pengajuan", khasPengajuanRoutes);
  apps.use("/api/khas/penetapan", khasPenetapanRoutes);
  apps.use(
    "/api/khas/uploads",
    express.static(path.join(khasBackendPath, "uploads")),
  );
}
...
apps.use(
  "/storage/news",
  express.static(path.join(__dirname, "public/uploads/news")),
);
apps.use(
  "/storage/hot_issue",
  express.static(path.join(__dirname, "public/uploads/hot_issue")),
);
apps.use(
  "/storage/photo",
  express.static(path.join(__dirname, "public/uploads/photo")),
);
apps.use(
  "/storage/structure",
  express.static(path.join(__dirname, "public/uploads/structure")),
);
apps.use(
  "/storage/filesupload",
  express.static(path.join(__dirname, "public/uploads/filesupload")),
);
...
apps.get("/nc", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/news_management/news_category/list.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/nc_add", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/news_management/news_category/add.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/nc_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/news_management/news_category/edit.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hi", (req, res) => {
  const role_id_users = req.cookies.roles_id;
  if (role_id_users == 1 || role_id_users == 2 || role_id_users == 4) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/hot_issue_management/hot_issue/list.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hi_add", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2 || role_id_users == 4) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/hot_issue_management/hot_issue/add.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hi_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2 || role_id_users == 4) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/hot_issue_management/hot_issue/edit.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hic", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/hot_issue_management/hot_issue_category/list.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hic_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/hot_issue_management/hot_issue_category/edit.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hic_add", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/hot_issue_management/hot_issue_category/add.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/hisc", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    ...
    res.sendFile(
      path.resolve(
        "./views/hot_issue_management/hot_issue_sub_category/list.html",
      ),
    );
  } else {
    res.redirect("/");
...
apps.get("/hisc_add", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    ...
    res.sendFile(
      path.resolve(
        "./views/hot_issue_management/hot_issue_sub_category/add.html",
      ),
    );
  } else {
    res.redirect("/");
...
apps.get("/hisc_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    ...
    res.sendFile(
      path.resolve(
        "./views/hot_issue_management/hot_issue_sub_category/edit.html",
      ),
    );
  } else {
    res.redirect("/");
...
apps.get("/fc", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/one_data_center/files_category/list.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/fc_add", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/one_data_center/files_category/add.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/fc_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/one_data_center/files_category/edit.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/cp", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/user_management/change_password/list.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/ip_address", (req, res) => {
  ...
  if (role_id_users == 1 || role_id_users == 2) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/user_management/whitelist/ipaddress.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/tk_form/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/profile/tentang_kami/tentang_kami_form.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/tk", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/profile/tentang_kami/tentang_kami.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/tk_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/profile/tentang_kami/tentang_kami_edit.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/es_form/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/profile/ekonomi_syariah/ekonomi_syariah_form.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/es", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/profile/ekonomi_syariah/ekonomi_syariah.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/web_title_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/pengaturan/identitas_web/edit_title.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/web_logo_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/pengaturan/identitas_web/edit_logo.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/web_header_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/pengaturan/identitas_web/edit_header.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.get("/web_color_edit/:id", (req, res) => {
  ...
  if (role_id_users == 1) {
    // kondisi.
    res.sendFile(
      path.resolve("./views/pengaturan/identitas_web/edit_color.html"),
    );
  } else {
    res.redirect("/");
  }
...
apps.post(
  "/es_updateabout",
  profile_path.single("logo_philosophy"),
  db.es_updateabouts,
);
...
apps.post(
  "/updateabout",
  profile_path.single("logo_philosophy_images"),
  db.updateabouts,
);
...
apps.get("/api_maps_kdeks", db.maps_kdeks);
...
apps.post(
  "/insertstructurekdeks",
  structure_path_kdeks.single("photo"),
  db.inserstructurekdeks,
);

apps.post(
  "/updatestructurekdeks",
  structure_path_kdeks.single("photo"),
  db.updatestructurekdeks,
);
...
apps.post(
  "/insertanggotakdeks",
  structure_path_kdeks.single("photo"),
  db.insertanggotakdeks,
);

apps.post(
  "/updateanggotakdeks",
  structure_path_kdeks.single("photo"),
  db.updateanggotakdeks,
);
...
apps.post(
  ...
  structure_path_kdeks.single("photo"),
  db.insertsubanggotakdeks,
);

apps.post(
  "/updatesubanggotakdeks",
  structure_path_kdeks.single("photo"),
  db.updatesubanggotakdeks,
);
...
apps.post("/insertnews", news_path.array("photo", 10), db.insertnews);

apps.post("/updatenews", news_path.array("photo", 10), db.updatenews);
...
apps.post(
  "/insertdirectorats",
  directorats_images.fields([
    { name: "images", maxCount: 1 },
    { name: "banners", maxCount: 1 },
  ]),
  db.insertdirectorats,
);
...
apps.post(
  "/updateinstitution",
  institutions_path.single("logo_member"),
  db.updateinstitution,
);

apps.post(
  "/insertinstitution",
  institutions_path.single("logo_member"),
  db.insertinstitution,
);
//::::::::::::::: Api & Query DB SOSMED ::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
...
apps.post(
  "/insertloginbanners",
  page_path.single("files_image"),
  db.insertloginbanner,
);

apps.post(
  "/updateloginbanners",
  page_path.single("files_image"),
  db.updateloginbanners,
);
...
apps.post(
  ...
  page_path.single("files_image"),
  db.insert_welcome_pages,
);

apps.post(
  "/update_welcome_pages",
  page_path.single("files_image"),
  db.update_welcome_pages,
);
...
//10371 prod
//13249 prod admin
apps.listen(port || 28789, () => {
  console.log(`CMS KNEKS berjalan di http://localhost:${port || 28789}`);
});
